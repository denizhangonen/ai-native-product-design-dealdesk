...
export function classifyDelivery(payload: unknown, channelId: string): SlackIntake {
  ...
  // Edits, joins, and deletions all arrive as subtypes and never start a request.
  if (subtype) return { kind: "ignored", reason: `message subtype ${subtype}` };
  if (channel !== channelId) return { kind: "ignored", reason: "different channel" };
  // Only a top-level message opens a request; thread replies are conversation.
  if (thread_ts) return { kind: "ignored", reason: "thread reply" };
