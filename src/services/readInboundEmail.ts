...
function readSelfSigned(body: string, headers: Headers): InboundEmailRead {
  ...
  const verified = verifyEmailSignature({
    body,
    timestamp: headers.get("x-dealdesk-timestamp"),
    signature: headers.get("x-dealdesk-signature"),
    secret,
  });
...
async function readFromResend(body: string, headers: Headers): Promise<InboundEmailRead> {
  ...
  // The delivery is genuinely from Resend, but the sender it names may not be.
  const sender = checkSenderAuthentication(content.authenticationResults);
  if (sender === "fail") {
    console.warn({ event: "email_sender_unauthenticated", emailId: event.data.email_id });
    return { kind: "unauthenticated_sender" };
  }
  if (sender === "unknown") {
    console.warn({ event: "email_sender_unverified", emailId: event.data.email_id });
  }
